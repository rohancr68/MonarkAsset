...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    await Promise.all([
      resend.emails.send({
        from:     'MonarkAsset <onboarding@resend.dev>',
    ...
    console.log(`📧  Enquiry from ${firstName} ${lastName} <${email}> sent successfully`);
...
setInterval(() => {
  ...
  if (!domain) return;
  https.get(`https://${domain}/health`, (pingRes) => {
    console.log(`🔄 Keep-alive ping: ${pingRes.statusCode}`);
  }).on('error', (err) => {
